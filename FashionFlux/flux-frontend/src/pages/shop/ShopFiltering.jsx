...
const ShopFiltering = ({filters , filtersState , setFiltersState , clearFilters}) => {
    ...
    // console.log(filters.priceRanges.label)
  return (
            ...
            {filters.priceRanges.map((range) => (
                <label key={range.label} className="capitalize cursor-pointer">
                    <input
                        ...
                        value={range.label} // Optional, used for debugging or accessibility
                        checked={filtersState.priceRange.label === range.label}
                        onChange={() => setFiltersState({ ...filtersState, priceRange: range })}
                    />
                    <span className="ml-1">{range.label}</span>
                </label>
            ))}
